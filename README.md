...
**Objectif :** Mettre en place la synchronisation NTP avec Chrony sur quatre distributions (Debian, Ubuntu, Rocky Linux, SUSE Linux) à l'aide de deux approches différentes.

### Référence par distribution

| Distribution | `ansible_os_family` | Module paquet | Paquet | Service systemd | Fichier de configuration |
|---|---|---|---|---|---|
| Debian / Ubuntu | `Debian` | `apt` | `chrony` | `chrony` | `/etc/chrony/chrony.conf` |
| Rocky Linux | `RedHat` | `dnf` | `chrony` | `chronyd` | `/etc/chrony.conf` |
| SUSE Linux | `Suse` | `zypper` | `chrony` | `chronyd` | `/etc/chrony.conf` |

> **À noter :** Le nom du service systemd diffère selon la famille : `chrony` sur Debian/Ubuntu et `chronyd` sur RedHat/SUSE.

---

### Exercice 1 — Approche « gros sabots » : `chrony-01.yml`

> **Consigne :** Utilisez les modules natifs `apt`, `dnf` et `zypper` avec des conditions `when` pour chaque distribution.
...
> Deux handlers distincts sont nécessaires car le nom du service systemd diffère : `chronyd` sur RedHat/SUSE, `chrony` sur Debian/Ubuntu.

---

### Exercice 2 — Approche par variables : `chrony-02.yml`

> **Consigne :** Utilisez le module générique `package` et trois variables `chrony_package`, `chrony_service` et `chrony_confdir` définies dans les `group_vars` de chaque distribution.
